use crate::types::{
  serde::DeError, BoltList, BoltMap, BoltNode, BoltPath, BoltPoint2D, BoltPoint3D, BoltRelation, BoltType,
...
mod tests {
  ...
  fn row_serializes_from_fields() {
      ...
      let data = BoltList::from(vec![
        ...
        BoltType::Map(
          [
            ...
            (BoltString::from("data"), BoltType::from(vec![1_u8, 3, 3, 7])),
          ]
          .into_iter()
          .collect(),
        ),
    ...
    let expected = Couple {
      p0: Person0 { name: "Alice".to_owned(), age: 42, score: 4.2, awesome: true, data: vec![4, 2] },
      p1: Person1 {
        name: "Bob",
        age: 1337,
        score: 13.37,
